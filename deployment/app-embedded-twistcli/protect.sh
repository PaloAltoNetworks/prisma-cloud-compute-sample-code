...
#Download the linux version of twistcli
#curl -sSLk -H "Authorization: Bearer $token" "$PCC_URL/api/v1/util/twistcli" > twistcli; chmod a+x twistcli;

#Download the macOS version of twistcli
curl -sSLk -H "Authorization: Bearer $token" "$PCC_URL/api/v1/util/osx/twistcli" > twistcli; chmod a+x twistcli;
...
./twistcli app-embedded embed --address $PCC_URL --token $token --output-file Dockerfile-defender.zip --data-folder /data --app-id $APPID Dockerfile

echo -e "unzip Dockerfile-defender.zip in the currect directory and call 'docker build -t $APPID:latest .' to build the image"
