...
#This curl command will generate a daemonset.yaml, that can be deploymed in clusters
curl -sSLk -H "authorization: Bearer $token" -X POST "$PCC_URL/api/v1/defenders/helm/twistlock-defender-helm.tar.gz" --data "$(generate_post_data)" -o twistlock-defender-helm.tar.gz
echo -e "Use file wistlock-defender-helm.tar.gz in the currect directory for deployment."
