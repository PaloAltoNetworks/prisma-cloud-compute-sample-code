# PolicyName:  PSS - Baseline - Deployment with containers that does not use safe sysctls
# Description: This policy identifies Deployments with containers that does not use safe sysctls
...
match[{"msg": msg}] {
    ...
    msg := sprintf("Deployment with containers using unsafe sysctls identified in %v", [name])
}
